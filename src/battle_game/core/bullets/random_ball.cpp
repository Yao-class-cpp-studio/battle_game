...
namespace battle_game::bullet {
  ...
  namespace {
    std::vector<std::function<void(battle_game::Unit*,glm::vec2, float, float, glm::vec2)>>
      crit_bullet_type_list;
    bool is_crit_bullet_type_set = false;
    #define ADD_SELECTABLE_CRIT_BULLET(BulletType)                                                                     \
      crit_bullet_type_list.push_back(                                                                                 \
      [](battle_game::Unit * my_unit, glm::vec2 position, float rotation, float damage_scale, glm::vec2 args) {        \
        my_unit->GenerateBullet<bullet::BulletType>(position,rotation,damage_scale,args);                              \
      });
    void set_crit_bullet_type() {
      // Welcome to add your bullet here!
      // Note: Only \ BulletType / having \ GenerateBullet<bullet::BulletType>(glm::vec2, float, float, glm::vec2) / will be accepted.
...
//      ADD_SELECTABLE_CRIT_BULLET(CovidBall);
  ...
  RandomBall::RandomBall(GameCore * core, uint32_t id, uint32_t unit_id,
                         uint32_t player_id, glm::vec2 position, float rotation,
                         float damage_scale, glm::vec2 velocity)

...
#include "battle_game/core/units/cooltank.h"

namespace battle_game::bullet {
Coolbullet::Coolbullet(GameCore *core,
                       uint32_t id,
                       uint32_t unit_id,
                       uint32_t player_id,
                       glm::vec2 position,
...
void Coolbullet::Update() {
  ...
  for (auto &unit : units) {
    ...
    if (unit.second->IsHit(position_)) {
      auto itself = game_core_->GetUnit(unit_id_);
      itself->Addhitnumber();

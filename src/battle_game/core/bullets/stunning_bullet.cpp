...
void StunningBullet::Update() {
  ...
  for (auto &unit : units) {
    ...
    if (unit.second->IsHit(position_)) {
      ...
      game_core_->PushEventBuffUnit(100, unit.first);
      should_die = true;
    }
  }
...
StunningBullet::~StunningBullet() {
  for (int i = 0; i < 5; i++) {
    game_core_->PushEventGenerateParticle<particle::Stunning>(

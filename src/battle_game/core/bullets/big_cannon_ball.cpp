...
BigCannonBall::BigCannonBall(GameCore *core,
                             uint32_t id,
                             uint32_t unit_id,
                             uint32_t player_id,
                             glm::vec2 position,
...
void BigCannonBall::Render() {
  SetTransformation(position_, rotation_, glm::vec2{0.15f});
  SetColor(game_core_->GetPlayerColor(player_id_));
  SetTexture("../../textures/particle3.png");
  DrawModel(0);
}

void BigCannonBall::Update() {

...
void SuperMinesweeper::Update() {
  ItemMove(3.0f, glm::radians(180.0f));
  TurretRotate();
  Fire();
}
void SuperMinesweeper::ItemMove(float move_speed, float rotate_angular_speed) {
  if (teleport_timer_) {
    teleport_timer_--;
  ...
  if (airborne_timer_) {
    airborne_timer_--;
    skills_[2].time_remain = airborne_timer_;
  }
  if (airborne_buffer_) {
    airborne_buffer_--;
  }
  if (airborne_control_buffer_) {
    airborne_control_buffer_--;
  }
  ...
  if (player) {
    ...
    if (input_data.key_down[GLFW_KEY_T] && !teleport_buffer_) {
      if (!teleport_timer_ && !is_teleport_) {
        is_teleport_ = true;
        skills_[4].is_prompt = true;
      } else if (is_teleport_) {
        is_teleport_ = false;
        skills_[4].is_prompt = false;
      }
      teleport_buffer_ = 20;
    }
    if (input_data.key_down[GLFW_KEY_E] && !airborne_buffer_) {
      if (!airborne_timer_ && !is_airborne_) {
        is_airborne_ = true;
        skills_[2].is_prompt = true;
        airborne_target_ = position_;
      } else if (is_airborne_) {
        is_airborne_ = false;
      ...
      if (timer_ % 30 == 0) {
        ...
        while (i < 120) {
          ...
          game_core_->PushEventGenerateParticle<particle::Signal>(
              airborne_target_, rotation_, velocity, 0.2f,
    ...
    if (input_data.key_down[GLFW_KEY_SPACE]) {
      if (is_airborne_) {
        landing_ = 60;
        is_airborne_ = false;
        airborne_timer_ = 1800;
        skills_[2].time_remain = 1800;
        skills_[2].is_prompt = false;
      }
    }
    if (landing_) {
      game_core_->PushEventGenerateParticle<particle::BulletStorage>(
          airborne_target_, rotation_, glm::vec2{0.0f, 0.0f},
          float(landing_) / 120,
          glm::vec4{0.0f, 0.0f, 0.0f, float(landing_) / 120 + 0.5}, 3.0f);
      if (landing_ == 1) {
        int j = 0;
        while (j < 60) {
          auto velocity =
        ...
        game_core_->PushEventGenerateParticle<particle::Expansion>(
            airborne_target_, rotation_, glm::vec2{0.0f, 0.0f}, 0.0,
            glm::vec4{1.0f, 0.5f, 0.0f, 0.0f}, 3.0f);
      }
      landing_--;
    }
    if (is_airborne_ && !airborne_control_buffer_) {
      auto player = game_core_->GetPlayer(player_id_);
      if (player) {
        auto &input_data = player->GetInputData();
        if (input_data.key_down[GLFW_KEY_UP]) {
          if (airborne_target_.y <= 9) {
            airborne_target_ += glm::vec2{0.0f, 1.0f};
          }
          airborne_control_buffer_ = 20;
        }
        ...
        if (input_data.key_down[GLFW_KEY_LEFT]) {
          if (airborne_target_.x >= -9) {
            airborne_target_ += glm::vec2{-1.0f, 0.0f};
          }
          airborne_control_buffer_ = 20;
        }
...
void SuperMinesweeper::TurretRotate() {
  auto player = game_core_->GetPlayer(player_id_);
  if (player) {
    timer_++;
    if (timer2_) {
...
void SuperMinesweeper::Fire() {
  if (is_second_ && !timer2_) {
    auto player = game_core_->GetPlayer(player_id_);
    if (player) {
...
void SuperMinesweeper::Fireout(glm::vec2 velocity) {
  int j = 0;
  while (j < 60) {
...
bool SuperMinesweeper::IsHit(glm::vec2 position) const {
  position = WorldToLocal(position);
  return position.x > -1.0f && position.x < 1.0f && position.y > -1.0f &&
         position.y < 1.0f;
}

const char *SuperMinesweeper::UnitName() const {
  return "Super \"minesweeper\"";
}

const char *SuperMinesweeper::Author() const {
  return "C161324";
}
}  // namespace battle_game::unit

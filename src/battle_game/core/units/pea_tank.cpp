...
// trying to build a tank that shoots in 4 directions simutaneously
...
void PeaTank::TankMove(float move_speed, float rotate_angular_speed) {
  ...
  if (fire_count_down_) {
    ...
    if (player) {
      ...
      if (input_data.mouse_button_down[GLFW_MOUSE_BUTTON_LEFT]) {
        float tr = turret_rotation_;
        for (int tmp = 0; tmp <= 3; tmp++) {
          tr += glm::pi<float>() * 0.5f;
          auto velocity = Rotate(glm::vec2{0.0f, 20.0f}, tr);
          GenerateBullet<bullet::CannonBall>(
              position_ + Rotate({0.0f, 1.2f}, tr), turret_rotation_,
              0.5 * GetDamageScale(), velocity);
        }

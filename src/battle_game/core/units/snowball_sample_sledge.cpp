...
snowball_sample_sledge::snowball_sample_sledge(GameCore *game_core,
  ...
  if (!~sledge_body_model_index) {
    ...
    {
      /* Sledge Body */
      sledge_body_model_index = mgr->RegisterModel(
          {
...
void snowball_sample_sledge::Render() {
  ...
  battle_game::DrawModel(sledge_body_model_index);
  battle_game::SetRotation(turret_rotation_);
  battle_game::DrawModel(sledge_turret_model_index);
}

void snowball_sample_sledge::Update() {
  SledgeMove(3.0f, glm::radians(180.0f));
  TurretRotate();
  Fire();
}
...
void snowball_sample_sledge::TurretRotate() {
  auto player = game_core_->GetPlayer(player_id_);
  if (player) {
    auto &input_data = player->GetInputData();
...
bool snowball_sample_sledge::IsHit(glm::vec2 position) const {
